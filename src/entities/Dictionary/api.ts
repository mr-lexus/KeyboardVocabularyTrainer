...
export const dictionaryApi = {
  ...
  create: async (name: string, emoji: string = '📝') => {
  ...
  updateEmoji: async (dictId: string, emoji: string) => {
    await db.dictionaries.update(dictId, { emoji });
  },
  updateName: async (dictId: string, name: string) => {
    await db.dictionaries.update(dictId, { name });
  },
